...
app.post('/api/exams/generate', async (req, res) => {
  ...
  try {
    ...
    const useBank = subject === '计算机';
    const { picked, remaining } = useBank ? pickFromBank(questionPlan, knowledgeScope, difficulty) : { picked: [], remaining: questionPlan };
    ...
    const withHashes = [...picked, ...llmQuestions].map((q) => {
      const withMeta = {
        ...q,
    ...
    const seenQ = new Set();
    const allQuestions = withHashes.filter((q) => {
      if (!q.hash) return true;
      if (seenQ.has(q.hash)) return false;
      seenQ.add(q.hash);
      return true;
    });

    const totalScore = allQuestions.reduce((sum, q) => sum + (Number(q.score) || 0), 0);
    const source = picked.length && llmQuestions.length ? 'mixed' : picked.length ? 'bank_only' : 'llm_only';
    ...
    db.read();
    db.data.examPapers = db.data.examPapers || [];
    const existing = (db.data.examPapers || []).find((p) => (p.hash || computePaperHash(p.courseId || courseId, p.questions || [])) === paper.hash);
    if (!existing) {
