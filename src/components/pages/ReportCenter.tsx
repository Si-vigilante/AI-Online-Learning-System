            ...
            {feedback && (
              <div className="mt-6 space-y-3">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.1fr)] gap-4 items-start">
                  <div className="p-4 bg-white rounded-lg border border-[#E9ECEF] space-y-2 shadow-sm">
                    <div className="flex items-center justify-between">
                      <h6 className="mb-0">我的提交</h6>
                      <span className="text-xs text-[#ADB5BD]">字数：{rawText.trim().length || 0}</span>
                    </div>
                    <p className="text-sm text-[#495057] whitespace-pre-line max-h-80 overflow-auto leading-relaxed">
                      {rawText || '未粘贴正文'}
                    </p>
                  </div>
                  <div className="p-4 bg-[#F8F9FA] rounded-lg border border-[#E9ECEF] space-y-3 shadow-sm">
                    <div className="flex items-center justify-between">
                      <div>
                        <h6 className="mb-0">优秀范例</h6>
                        <p className="text-xs text-[#ADB5BD]">{reportExemplar.title}</p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-2 bg-[#F3F0FF] rounded-lg">
                        <p className="text-[11px] text-[#845EF7] mb-1">相似度</p>
                        <div className="text-xl font-semibold text-[#5F3DC4]">{diff.similarity}%</div>
                      </div>
                      <div className="p-2 bg-[#E7F5FF] rounded-lg">
                        <p className="text-[11px] text-[#4C6EF5] mb-1">关键段落覆盖</p>
                        <div className="text-xl font-semibold text-[#1C7ED6]">{diff.coverage}%</div>
                      </div>
                    </div>
                    <p className="text-sm text-[#495057] whitespace-pre-line max-h-80 overflow-auto leading-relaxed">
                      {reportExemplar.rawText}
                    ...
                    <div>
                      <p className="text-xs text-[#ADB5BD] mb-1">建议补充的关键表达</p>
                      {diff.missing.length ? (
                        <ul className="space-y-1 text-sm list-disc list-inside text-[#212529]">
                          {diff.missing.map((item, idx) => (
                            <li key={idx}>{item}</li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-sm text-[#51CF66]">已覆盖范例核心段落</p>
                      )}
                    </div>
                  </div>
                </div>
